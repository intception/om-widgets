...
(defn- check
  ...
  (reify
    om/IRenderState
    (render-state [this {:keys [label id path disabled on-change checked-value unchecked-value toggle-value]}]
      (html
...
;; Public
(defn checkbox
  [app path {:keys [label id disabled class-name on-change checked-value unchecked-value toggle-value] :as opts
